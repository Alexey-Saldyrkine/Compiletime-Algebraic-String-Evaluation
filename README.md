...
This is a single header that provides the capability to evaluate algebraic expressions at compile-time.
...
[godBolt link](https://godbolt.org/z/E4zGjr4eM)
...
## tstring and how expressions are declared

tstring is a wrapper around std::integer_subsequence<char,chrs...>.<br>
A string of characters can be converted to a tstring value through the user-defined literal _tstr. Then converted to a type using decltype.<br>

example: 
```cpp
using expr = decltype("2+5"_tstr);
```
expr is an tstring type that represents the expression 2+5, which upon evaluation will result in 7.

## userContext

userContext is a type that hold information about other expression definitions that could be used in an evaluation of an expression.<br>
...
### Value type
When declaring a userContext, an optional value type T can be provided.<br>
...
If no type T is provided, then type T defaults to int.<br>
examples:<br>
```cpp
...
```
__NOTE:__ only integer types can be used as value types;

### Adding definitions to a context

userContext by default holds no expression definitions.<br>
New definitions can be added through the member type "add" of userConext.<br>
...
```
such "add"s can be chained in a single using declaration:
```cpp
...
Expressions can be evaluated to their result value using the calcString template value.<br>
calcString will return a value of type 'value type';

### Value type 
If a userContext is given as the second parameter, then the value type is got from the userContext.<br>
If a value type is given as the second parameter, then it will be used.<br>
If no second parameter is given, then the value type defaults to int.<br>
examples:
```cpp
constexpr int res = calcString<expr>; // the expression will be evaluated with the default value type int.

constexpr long int res = calcString<expr,long int>; // the expression will be evaluated with the value type being long int.

using context = userContext<long long int>;

constexpr long long int res = calcString<expr,context>; // the expression will be evaluated with the value type of context (i.e. long long int).
```
### Detail
...
When the mathEngine receives an expression the following steps will happen:
- The expression will be searched for the leftmost operator in order of operators precedence.<br>
  - If an operator is found, than the expression will split by the operator into 2 operand expressions.<br>
...
- If no operator is found, then the expression is a value expression.<br>
1. The value expression is attempted to be converted int a value of type 'value type;.<br>
    - If the conversion is successful, then the value is returned
...
3. The expression is tested for the presence of the '(' character, if it is present, then the expression is considered a function call.<br>
    - The expression is split into the function name and a list of parameters. If the function is present in the context, then all the parameters are evaluated in order into the value list.<br>
...
4. Else, the expression must be a constant.
    ...
    - If it is not present, then an error occurs.

__Note:__ an empty expression (i.e. "") evaluates to 0.<br>
This is used primarily to define negative numbers in an expression. i.e. the expression "-1" is treated as "0-1" which evaluates to -1.
...
3. '^'
4. '(' ')'
Operators of the same level are evaluated left to right as they appear in an expression.
...
Where \[name of constant] is a string of characters that will identify the constant.<br>
...
\[expression] must be a valid string of characters that can be evaluated by calcString.<br>
The \[expression] can refer to other constants and functions, so long as they are defined in the same context, by the time that the context is used by calcString.
...
The amount of variables must be more than zero.<br>
